...
import com.audit.agriin.Domains.DTOs.Oauth.TokenDto;
import com.audit.agriin.Domains.DTOs.Oauth.UrlDto;
...
import com.audit.agriin.Repositories.ImageStorageRepository;
import com.audit.agriin.Repositories.UserRepository;
import com.audit.agriin.Services.Specification.JwtService;
import com.audit.agriin.Services.Specification.OauthService;
...
public class OauthServiceImpl implements OauthService {
    ...
    private User createUserFromPayload(GoogleIdToken.Payload payload) {
        // TODO: upload user image to cloud storage and save the URL in the user's storage

        User newUser = User.builder()
                .firstname(payload.get("given_name").toString())
                .lastname(payload.get("family_name").toString())
                .email(payload.getEmail())
                .userGroups(Set.of(groupRepository.findGroupByName("DEFAULT_USER")))
                .password(passwordEncoder.encode("googlePassword"))
                .accountNonLocked(true)
                .enabled(false)
                .failedLoginAttempts(0)
                .loginDisabled(false).build();
